    ...
    <script>
        ...
        $.ajax({
            ...
            success:function(campList){
                ...
                $('#res-gen-search').html('<tr><th>camp_id</th><th>camp_name</th><th>cost</th><th>start_date</th><th>end_date</th></tr>');
                for(var i = 0; i < campList.length; i++){
                    $('#res-gen-search').append('<tr><td>'+campList[i]['camp_id']+'</td><td>'+campList[i]['camp_name']+'</td><td>'+campList[i]['cost']+'</td><td>'+campList[i]['start_date']+'</td><td>'+campList[i]['end_date']+'</td><td>'+ '<button class="btn btn-success btn-sm"><span>Register</span></button>' +'<button style="background-color:#0BBA36; color:white;" class="btn btn-view btn-sm"><span>View Description</span></button>'+'</td></tr>');
                }
                $('#res-gen-search').show();
            },
            error:function(){
